...
const fs = require('fs');
const writeStream = fs.createWriteStream(__dirname + '/reviewsmongo.tsv');
...
function generateReviews(writer) {
  ...
  function write() {
    ...
    do {
      ...
      const users_id = faker.random.number({max: 10000000});
      const restaurant_id = faker.random.number({max: 10000000});
